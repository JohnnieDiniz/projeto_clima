...
homeBtn.addEventListener('click', () => {
    ...
    document.body.className = ''; // Remove qualquer tema de clima (volta ao padrão)
    renderHistory();
    cityInput.focus(); 
});
...
// Fluxo Principal de Busca por Texto
...
// Interage com a API de Geocodificação
...
// Interage com a API de Previsão do Tempo
...
// Dicionário de Descrição do Clima
...
// NOVO: Aplica os temas dinâmicos dependendo do código do clima
function applyDynamicTheme(code) {
    document.body.className = ''; // Limpa as classes atuais
...
// Funções de Histórico (Local Storage)
...
function handleError(error) {
    ...
    historyContainer.classList.add('hidden');
    document.body.className = ''; // Reseta o fundo em caso de erro
...
// NOVO: Exibe o estado de carregamento com a Animação CSS (Spinner)
function setLoadingState(isLoading) {
    if (isLoading) {
        document.body.className = ''; // Limpa o fundo antigo
        weatherResult.innerHTML = `
